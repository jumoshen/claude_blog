...
const fetchPosts = async () => {
  try {
    ...
    if (res.code === 0 && res.data) {
      ...
      cards.value = posts.slice(0, 3).map(post => ({
        id: post.id,
        slug: post.slug,
    ...
    // 填充占位卡片
    cards.value = [
      { id: null, slug: null, title: '加载失败', flipped: false },
      { id: null, slug: null, title: '加载失败', flipped: false },
      { id: null, slug: null, title: '加载失败', flipped: false }
    ]
  }
...
.theme-pixel .card-back {
  ...
  box-shadow: 4px 4px 0 #000, inset 0 0 0 2px var(--accent);
}
...
.theme-qver .card-back .suit {
  color: #a78bfa;
  text-shadow: 0 0 10px #a78bfa, 0 0 20px #7c6aff;
}
