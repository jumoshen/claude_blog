...
// GetAdminUserPermissions 获取管理员用户的权限列表
func (r *Repository) GetAdminUserPermissions(userID int64) ([]string, error) {
	// 从 admin_users 表获取用户
	var user struct {
		Role string `gorm:"column:role"`
	}
	if err := r.db.Table("admin_users").Select("role").Where("id = ?", userID).First(&user).Error; err != nil {
		return nil, err
	}

	// 从 admin_roles 表获取角色权限
	var role model.AdminRole
	if err := r.db.Where("name = ?", user.Role).First(&role).Error; err != nil {
		return nil, err
	}

	// role.Permissions 是 JSON 格式的字符串，需要解析
	return parseJSONArray(role.Permissions), nil
}

// parseJSONArray 简单的 JSON 数组解析
...
// ListRoles 获取所有角色
func (r *Repository) ListRoles() ([]RoleInfo, error) {
...
func (r *Repository) GetRoleByID(id uint) (*RoleInfo, error) {
	var role model.AdminRole
...
// CreateRole 创建角色
func (r *Repository) CreateRole(name string, permissions []string) (*RoleInfo, error) {
	permissionsJSON := "[]"
...
// UpdateRole 更新角色
func (r *Repository) UpdateRole(id uint, name string, permissions []string) (*RoleInfo, error) {
	var role model.AdminRole
	if err := r.db.First(&role, id).Error; err != nil {
		return nil, err
	}
